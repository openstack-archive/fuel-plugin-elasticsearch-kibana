...
$haproxy_port = hiera('lma::elasticsearch::haproxy_port')
...
$haproxy_service = hiera('lma::elasticsearch::haproxy_service')
openstack::ha::haproxy_service { $haproxy_service:
  order                  => '920',
  listen_port            => $es_port,
...
openstack::ha::haproxy_service { 'kibana':
  ...
  balancermember_port    => $kibana_port,
  balancermember_options => 'check inter 10s fastinter 2s downinter 3s rise 3 fall 3',
  haproxy_config_options => {
    'option'  => ['httplog', 'http-keep-alive', 'prefer-last-server', 'dontlog-normal'],
    'balance' => 'roundrobin',
