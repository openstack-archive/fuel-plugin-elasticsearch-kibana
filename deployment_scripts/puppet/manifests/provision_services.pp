...
if $kibana_tls['enabled'] {
  ...
  if $two_links {
    ...
    \"description\":\"Dashboard for visualizing logs and notifications (${kibana_hostname}: ${vip}:${kibana_port})\",\
    \"url\":\"${protocol}://${kibana_hostname}:${kibana_port}/\"}"
    $kibana_link_viewer_data = "{\"title\":\"Kibana (Viewer role)\",\
    \"description\":\"Dashboard for visualizing logs and notifications (${kibana_hostname}: ${vip}:${kibana_viewer_port})\",\
