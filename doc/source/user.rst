...
If you cannot access to Kibana dashboard or you get no data in dashboards,
...
1. First, check that the Elasticsearch is running properly using *curl*::
   ...
   Where *HOST* is the same IP address of the Kibana dashboard.
...
2. Check the status of the VIP and HAProxy::

    # On elasticsearch-kibana node
...
3. If the VIP or HAProxy is down, restart them::
...
9. Check if the nodes are able to connect to the Elasticsearch server(s) through
   the VIP  on port *9200*.
